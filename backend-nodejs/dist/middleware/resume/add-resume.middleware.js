...
exports.default = Add_Resume_Middleware;
function Add_Resume_Middleware(req, res, next) {
    try {
        const { template } = req.body;
        if (template == null || typeof template !== "string" || !(["template1", "template2"].includes(template))) {
            res.status(200).json({
                success: false, message: "Invalid template name",
            });
            return;
        }
        if (template === "template1") {
            const message = Add_Resume_Middleware_Template_1(req.body);
            if (message !== null) {
        ...
        if (template === "template2") {
            const message = Add_Resume_Middleware_Template_2(req.body);
            if (message !== null) {
                res.status(200).json({ success: false, message: message });
                return;
            }
...
function Add_Resume_Middleware_Template_1(data) {
    if (!data.title || typeof data.title !== "string")
        return "Field 'title' is required and must be a string.";
    if (!data.email || typeof data.email !== "string")
        return "Field 'email' is required and must be a string.";
    ...
    if (linkedin && !/^https?:\/\/(www\.)?linkedin\.com\/.+/.test(linkedin))
        return "Personal 'linkedin' must be a valid LinkedIn URL.";
    if (github && !/^https?:\/\/(www\.)?github\.com\/.+/.test(github))
        return "Personal 'github' must be a valid GitHub URL.";
    ...
    if (data.projectDetails) {
        ...
        for (let i = 0; i < data.projectDetails.length; i++) {
            ...
            if (!Array.isArray(proj.description) || proj.description.some((d) => typeof d !== "string"))
    ...
    if (data.experienceDetails) {
        ...
        for (let i = 0; i < data.experienceDetails.length; i++) {
            ...
            if (!Array.isArray(exp.description) || exp.description.some((d) => typeof d !== "string"))
                return `Experience[${i}].description must be an array of strings.`;
        }
    }
    return null;
}
function Add_Resume_Middleware_Template_2(data) {
    // ------------------ Personal Details ------------------
    if (!data.personalDetails)
        return "Personal details are required.";
    ...
    if (data.experienceDetails) {
        ...
        for (let i = 0; i < data.experienceDetails.length; i++) {
            ...
            if (!Array.isArray(exp.keySkills) || exp.keySkills.some((skill) => typeof skill !== "string"))
    ...
    if (data.projectDetails) {
        ...
        for (let i = 0; i < data.projectDetails.length; i++) {
            ...
            if (!Array.isArray(proj.description) || proj.description.some((d) => typeof d !== "string"))
                return `Project[${i}].description must be an array of strings.`;
            if (!Array.isArray(proj.keySkills) || proj.keySkills.some((skill) => typeof skill !== "string"))
