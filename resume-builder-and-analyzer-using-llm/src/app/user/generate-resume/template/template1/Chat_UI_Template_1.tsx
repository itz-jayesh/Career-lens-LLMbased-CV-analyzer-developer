...
export default function Chat_UI_Template_1({ data, stage }: { data: Template_1_Type, stage: number }) {
    ...
    const section: { [key: number]: string } = {
        ...
        6: "Certification Details Section",
        7: "Extra Curricular Activities Details Section",
    }
    ...
    const sendMessage = async () => {
        ...
        if (stage === 2) userdata = JSON.stringify({ summary: data.skillDetails })
        ...
        if (stage === 6) userdata = JSON.stringify({ summary: data.certificationDetails })
        if (stage === 7) userdata = JSON.stringify({ summary: data.extraCurricularDetails })
